...
def IdeogramArc(start=0, end=60, radius=1.0, width=0.2, ax=None, color=(1,0,0),see_through=True):
    ...
    if ax == None:
        ...
        if see_through:
            patch = patches.PathPatch(path, facecolor=color+(0.35,), edgecolor=color+(0.35,), lw=LW)
...
def ChordArc(start1=0, end1=60, start2=180, end2=240, radius=1.0, chordwidth=0.7, ax=None, color=(1,0,0),see_through=True):
    # start, end should be in [0, 360)
    if start1 > end1:
...
def selfChordArc(start=0, end=60, radius=1, chordwidth=0.7, ax=None, color=(1,0,0), see_through=True):
    # start, end should be in [0, 360)
    if start > end:
        start, end = end, start
    start *= np.pi/180.
...
def chordDiagram(X, ax, colors=None, width=0.1, pad=2, chordwidth=0.7):
    ...
    for i in range(len(x)):
        ...
        if -30 <= angle <= 210:
            ...
            angle -= 270
        nodePos.append(tuple(polar2xy(1.1, 0.5*(start+end)*np.pi/180.)) + (angle,)+(start,)+(end,))
